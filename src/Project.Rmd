...
```{r}
# convert categorical variables to as.factor
...
plot(xBalance(HGOVCUR ~ SEX + AGE2004 + as.factor(RACEETHN) + as.factor(MOMEDUC) + as.factor(MOMSTAT) + ZIPURB -1, strata= list(unstrat=NULL), data=dat))
```
...
dat$prop <- glm(HGOVCUR ~ SEX + AGE2004 + as.factor(RACEETHN) + as.factor(MOMEDUC) + as.factor(MOMSTAT) + ZIPURB, family=binomial, 
...
# average absolute difference in propensity score, after caliper matching
...
plot(xBalance(HGOVCUR ~ SEX + AGE2004 + as.factor(RACEETHN) + as.factor(MOMEDUC) + as.factor(MOMSTAT) + ZIPURB + prop- 1, 
...
```{r}
# match each treated to 12 controls, w/ caliper
mat.5 <- addcaliper(mat.1, z=dat$HGOVCUR, p=dat$prop, caliper=0.1) # same caliper as before, different name
ms.5 <- pairmatch(mat.5, controls = 12, data=dat) # can only match to 12 because not enough controls for each treated

# covariate balance before and after 1-12 matching
plot(xBalance(HGOVCUR ~ SEX + AGE2004 + as.factor(RACEETHN) + as.factor(MOMEDUC) + as.factor(MOMSTAT) + ZIPURB + prop - 1,

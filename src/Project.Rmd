...
# Compute propensity
aipw_data$propensity <- glm(HGOVCUR ~ SCSELF + RCNOW + CPSNOW + PAAHOME + as.factor(SEX) + AGE2004 + as.factor(RACEETHN) + as.factor(MOMEDUC) + as.factor(MOMSTAT) + as.factor(ZIPURB), data=aipw_data, family = "binomial")$fitted.values

# Create estimators
mu1 <- lm(SEGRADES ~ SCSELF + RCNOW + CPSNOW + PAAHOME + as.factor(SEX) + AGE2004 + as.factor(RACEETHN) + as.factor(MOMEDUC) + as.factor(MOMSTAT) + as.factor(ZIPURB) + HGOVCUR, data=aipw_data, subset = HGOVCUR==1)

mu0 <- lm(SEGRADES ~ SCSELF + RCNOW + CPSNOW + PAAHOME + as.factor(SEX) + AGE2004 + as.factor(RACEETHN) + as.factor(MOMEDUC) + as.factor(MOMSTAT) + as.factor(ZIPURB) + HGOVCUR, data=aipw_data, subset = HGOVCUR==0)

aipw_data$mu1pred <- predict(mu1, aipw_data)
aipw_data$mu0pred <- predict(mu0, aipw_data)

# Compute AIPW ATE estimator
mu1_dr <- mean(aipw_data$HGOVCUR * (aipw_data$SEGRADES - aipw_data$mu1pred) / aipw_data$propensity + aipw_data$mu1pred)

mu0_dr <- mean((1 - aipw_data$HGOVCUR) * (aipw_data$SEGRADES - aipw_data$mu0pred) / aipw_data$propensity + aipw_data$mu0pred)
